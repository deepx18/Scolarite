    ...
    <td class="px-6 py-5">
        <div class="flex items-center gap-3">
            <div class="w-8 h-8 rounded-lg {{ $typeStyle['bg'] }} flex items-center justify-center">
                <span class="material-symbols-outlined {{ $typeStyle['color'] }} text-lg">{{ $typeStyle['icon'] }}</span>
            </div>
            <span class="font-medium">{{ $request->typeLabel() }}</span>
        </div>
    </td>
    <td class="px-6 py-5 text-secondary">{{ $request->submitted_at?->format('M d, Y') ?? $request->created_at->format('M d, Y') }}</td>
    <td class="px-6 py-5">
        <div class="flex justify-center">
            <span class="px-3 py-1 rounded-full text-[10px] font-extrabold uppercase tracking-wider {{ $statusStyle['bg'] }} {{ $statusStyle['text'] }} border {{ $statusStyle['border'] }}">
                {{ $request->statusLabel() }}
            </span>
        </div>
    </td>
