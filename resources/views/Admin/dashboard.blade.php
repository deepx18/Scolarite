<x-admin.layout>
        ...
        <div class="flex flex-col gap-6">
                    ...
                    <div
                        ...
                        <div>
                            <h2 class="text-lg font-semibold text-blue-950 dark:text-white">
                                {{ __('admin.request_volume_by_type') }}</h2>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6">
                            <div class="rounded-3xl bg-slate-50 dark:bg-slate-900 p-4">
                                <p class="text-xs uppercase tracking-[0.28em] text-slate-500">
                                    {{ __('admin.peak_hour') }}</p>
                                <p class="mt-3 font-semibold text-slate-900 dark:text-white">
                                    {{ __('admin.peak_hour_example') }}</p>
                            </div>
                            <div class="rounded-3xl bg-slate-50 dark:bg-slate-900 p-4">
                                <p class="text-xs uppercase tracking-[0.28em] text-slate-500">
                                    {{ __('admin.average_processing') }}</p>
                                <p class="mt-3 font-semibold text-slate-900 dark:text-white">
                                    {{ __('admin.average_processing_example') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="flex items-start gap-4">
                                ...
                                <div class="flex-1">
                                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1">
                                        ...
                                        <span
                                            class="inline-flex rounded-full bg-slate-100 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.24em] text-slate-600 dark:bg-slate-800 dark:text-slate-300">{{ $req->statusLabel() }}</span>
                                    </div>
                                    <p class="mt-1 text-sm text-slate-500">{{ $req->typeLabel() }} ·
            ...
            <div class="grid grid-cols-1 xl:grid-cols-2 gap-4">
                ...
                <div
                    class="rounded-3xl bg-white dark:bg-slate-950 p-6 shadow-sm border border-slate-200 dark:border-slate-800">
                    <h2 class="text-lg font-semibold text-blue-950 dark:text-white"
                        style="font-family: 'Manrope', sans-serif;">{{ __('admin.priority_actions.title') }}</h2>
                    ...
                    <div class="mt-4 space-y-4">
                        @forelse($pendingActions as $action)
                            <div class="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-800 rounded-lg">
                                <div>
                                    <p class="font-semibold text-slate-900 dark:text-white">
                                        {{ $action->student->first_name }} {{ $action->student->last_name }}</p>
                                    <p class="text-sm text-slate-500">{{ $action->type }}</p>
                                </div>
                                <span
                                    class="inline-flex items-center rounded-full bg-red-100 px-2 py-1 text-xs font-semibold text-red-700">{{ __('admin.priority_actions.delayed_label') }}</span>
                            </div>
                        @empty
                ...
                <div
                    class="rounded-3xl bg-white dark:bg-slate-950 p-6 shadow-sm border border-slate-200 dark:border-slate-800">
                    <h2 class="text-lg font-semibold text-blue-950 dark:text-white"
                        style="font-family: 'Manrope', sans-serif;">{{ __('admin.status_distribution.title') }}</h2>
                        ...
                        @foreach($statusDistribution as $dist)
                            <div>
                                <div class="flex items-center justify-between text-sm mb-2">
                                    ...
                                    <span
                                        class="font-semibold text-slate-900 dark:text-white">{{ $dist['percent'] }}%</span>
                                </div>
                                <div class="h-3 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800">
                                    <div class="h-full rounded-full"
                                        style="width: {{ $dist['percent'] }}%; background-color: #002045;"></div>
                                </div>
                            </div>
                        @endforeach
                    </div>
    ...
    @include('components.admin.export-students-modal')
</x-admin.layout>